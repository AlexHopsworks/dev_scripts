...
if [ ${SSH_ENTRY} == 2 ]; then
  SSH_BEGIN=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_ALIAS}" | cut -d':' -f1 | gsed -n 1p)
  SSH_END=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_ALIAS}" | cut -d':' -f1 | gsed -n 2p)
...
if [ ${PFWD_ENTRY} == 2 ]; then
  SSH_PFWD_BEGIN=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_PFWD_ALIAS}" | cut -d':' -f1 | gsed -n 1p)
  SSH_PFWD_END=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_PFWD_ALIAS}" | cut -d':' -f1 | gsed -n 2p)
