...
while read -r line; do
  ...
  SSH_ENTRY=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_ALIAS}_vagrant" | cut -d':' -f1 | wc -l)
  if [ ${SSH_ENTRY} == 2 ]; then
    SSH_BEGIN=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_ALIAS}_vagrant" | cut -d':' -f1 | sed -n 1p)
    SSH_END=$(cat ${SSH_CONFIG_FILE} | grep -n "Host ${SSH_ALIAS}_vagrant" | cut -d':' -f1 | sed -n 2p)
